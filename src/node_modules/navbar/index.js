...
function navbar (opts = default_opts, protocol) {
  ...
  // sm nav buttons
  const consortium_btn = icon_button({ src: icon_consortium }, button_protocol)
  ...
  // Page List Buttons
  const text_btns = [
    { element: text_button({ text: 'HOME' }, button_protocol) },
    { element: text_button({ text: 'PROJECTS' }, button_protocol) },
    { element: text_button({ text: 'GROWTH PROGRAM' }, button_protocol) },
    { element: text_button({ text: 'TIMELINE' }, button_protocol) }
  ]
  ...
  // protocol
  function button_protocol (handshake, notify, mid = 0) {
    console.log('navar:subprotocol', { handshake, notify, mid })
    PROTOCOL.controls[notify] = handshake
    if (notify) return [listen, id]
    function listen (message) {
