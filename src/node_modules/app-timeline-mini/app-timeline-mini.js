...
function app_timeline_mini (opts = default_opts, protocol) {
  ...
  // ID + JSON STATE
  // ----------------------------------------
  const name = `app_timeline_mini-${count++}`
  // ----------------------------------------
  // OPTS
  // ----------------------------------------
  const { data } = opts
  ...
  timeline_wrapper.append(...cards_data.map(timeline_card).map(shadowfy()))
  ...
  main_wrapper.append(shadowfy()(scrollbar({data: data}, timeline_mini_protocol)))
